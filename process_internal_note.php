...
try {
    ...
    if (!empty($novo_status)) {
        ...
        if (in_array($novo_status, $allowed_status) && $novo_status !== $current_chamado_status) {
            if (!$change_details_added) { $corpo_nota_final .= "<br><br><hr>"; $change_details_added = true; }
            $corpo_nota_final .= "<em>O status do chamado foi alterado de <strong>" . htmlspecialchars($current_chamado_status) . "</strong> para <strong>" . htmlspecialchars($novo_status) . "</strong> por <strong>" . htmlspecialchars($current_user_name) . "</strong>.</em>";
        }
    }
