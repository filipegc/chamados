...
try {
    ...
    $stmt = $conn->prepare("SELECT assunto, status, categoria_id FROM chamados WHERE id = ?");
    ...
    $nome_remetente = ($atendente_info['email_sender_name_type'] === 'global_name') ? ($conn->query("SELECT config_value FROM sistema_config WHERE config_key = 'support_sender_name'")->fetch_assoc()['config_value'] ?? 'Equipe de Suporte') : $atendente_info['nome'];
    ...
    // ... (lógica para salvar anexos e imagens no DB) ...

    // CORRIGIDO: Lógica para criar notas internas automáticas agora inclui um Message-ID único.
    ...
    if ($categoria_id !== null && $categoria_id !== $current_chamado_categoria_id) {
        // CORRIGIDO: Implementa a busca de nomes de categoria para a nota.
